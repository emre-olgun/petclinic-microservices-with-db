...
resource "aws_security_group" "petclinic-mutual-sg" { #buradaki sec grup portlarını K8s dokumanından aldık
...
resource "aws_iam_role" "petclinic-master-server-s3-role" { #s3 e read only yetkisi verdik. HELM reposu olarak s3 kullanacağız Oraya ulaşsın diye S3 yetkisi verdik.
...
resource "aws_instance" "kube-master" {
    ...
    key_name = "clarus" # Bura sabit kalıcak ileride bir komut ikle değiştireceğiz.
    subnet_id = "subnet-0699749dc4266dba6"  # select own subnet_id of us-east-1a
